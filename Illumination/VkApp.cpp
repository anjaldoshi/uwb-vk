...
void VkApp::initialize(const std::string& appName)
{
	...
	createGround();
	mGroundXForm.scale = glm::vec3(100.f);
	mGroundXForm.position -= glm::vec3(0.0f, 7.5f, 0.0f);
...
void VkApp::lightControls()
{
	...
	if (mLightOrbit) {
		mLightUBO.lights[1].position = glm::vec4(orbitRadius * glm::sin(orbitSpeed * mElapsedTime),
				0.0f,
				orbitRadius * glm::cos(orbitSpeed * mElapsedTime), 1.0f);

		for (uint32_t lightIndex = 2; lightIndex < mTotalLights; lightIndex++) {
			glm::quat rot = glm::angleAxis(glm::radians(180.f), glm::vec3(0.0f, 1.0f, 0.0f));
			mLightUBO.lights[lightIndex].position = rot * mLightUBO.lights[1].position;
		}
	}
...
void VkApp::setupCamera()
{
	...
	mCamera->position = glm::vec3(0.0f, 3.0f, 40.0f);
}
...
void VkApp::setupLights()
{
	...
	mLightUBO.lights[2].lightType   = LightType::Spot;
	mLightUBO.lights[2].ambient     = glm::vec4(0.01, 0.01, 0.01, 1.0);
	mLightUBO.lights[2].diffuse     = glm::vec4(16.0f, 0.0f, 0.0f, 1.0f);		//red light
	mLightUBO.lights[2].specular    = glm::vec4(16.0f, 16.0f, 16.0f, 1.0f);		
	mLightUBO.lights[2].cutOff		= glm::cos(glm::radians(45.0f));
	mLightUBO.lights[2].outerCutOff = glm::cos(glm::radians(50.0f));
...
void VkApp::createGround()
{
	...
	mRenderSystem.createShader(groundShaderSet.vertShader, BOX_VERT_SHADER_PATH, VK_SHADER_STAGE_VERTEX_BIT);
	mRenderSystem.createShader(groundShaderSet.fragShader, BOX_FRAG_SHADER_PATH, VK_SHADER_STAGE_FRAGMENT_BIT);
	...
	//instantiate (flush bindings, create pipeline)
	std::cout << "Instantianting a ground" << std::endl;
