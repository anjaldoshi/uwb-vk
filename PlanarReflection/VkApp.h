...
const std::string WINDOW_TITLE = "PlanarReflection";
...
//mirror
const std::string MIRROR_TEXTURE_PATH		= "Resources/Textures/Mirror/mirror.jpg";
const std::string MIRROR_MESH_PATH			= "Resources/Meshes/ground.mesh";
const std::string MIRROR_VERT_SHADER_PATH	= "Resources/Shaders/mirror.vert.spv";
const std::string MIRROR_FRAG_SHADER_PATH	= "Resources/Shaders/mirror.frag.spv";
...
private:
	...
	//Mirror Renderable
	std::shared_ptr<Renderable> mMirror;							///< A Mirror Renderable to project shadows on
	std::shared_ptr<UBO> mMirrorMVPBuffer;							///< A UBO for sending the Mirror's MVP matrices to the shaders
	Transform mMirrorXForm;											///< The mirror's transform

	//Shadow Info
	std::shared_ptr<UBO> mReflectionMVPBuffer;							///< A UBO for holding sending information about the shadow projection to make a shadow map
...
private:
	...
	*/
	...
	/** @brief Create a ground plane to map reflections on */
	void createMirror();
	...
	*/
	...
	void updateMirrorMVPBuffer(const UBO& mvpBuffer,
		const Transform& renderableXform,
		const Camera& cam);

	/** @brief updates the MVP buffer for a light source and sets the VP buffer for the reflection it casts 
		@param light The light casting a shadow
	*/
	void updateReflectionMVP(const Camera& cam, const Light & light);
};
